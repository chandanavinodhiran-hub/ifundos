...
import {
  ...
  RefreshCw, Hash, ChevronDown, ChevronUp, Shield,
} from "lucide-react";
...
function getActionColor(action: string): string {
  ...
  if (action.includes("LOGIN") || action.includes("AUTH")) return "bg-gray-100 text-gray-600 border-gray-200";
  ...
  if (action.includes("SHORTLISTED") || action.includes("QUESTIONNAIRE")) return "bg-cyan-100 text-cyan-800 border-cyan-200";
  if (action.includes("CONTRACT") || action.includes("DISBURSEMENT")) return "bg-emerald-100 text-emerald-800 border-emerald-200";
  if (action.includes("BOOST")) return "bg-amber-100 text-amber-800 border-amber-200";
...
const ACTIONS = [
  ...
  "AI_SCORING_COMPLETED", "APPLICATION_SHORTLISTED", "APPLICATION_SUBMITTED",
  "QUESTIONNAIRE_SENT", "CONTRACT_AWARDED",
];
...
export default function AuditTrailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Tamper-evident log of all platform actions ({total.toLocaleString()} total events)
          </p>
        </div>
      ...
      {chainValid !== null && (
        <Card className={`rounded-xl ${chainValid ? "border-green-300 bg-green-50" : "border-red-300 bg-red-50"}`}>
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-green-700">
                    All {chainEventCount ?? total} audit events are cryptographically linked and intact.
                  </p>
                </div>
              </>
            ) : (
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <Badge variant="outline" className="bg-gray-50 text-gray-600 border-gray-200">
          Auth: {categoryCounts.LOGIN}
        </Badge>
        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
          Applications: {categoryCounts.CREATE + categoryCounts.UPDATE}
        </Badge>
        <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
          AI Scoring: {categoryCounts.VERIFY}
        </Badge>
        <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">
          Governance: {categoryCounts.OTHER}
        </Badge>
        {categoryCounts.DELETE > 0 && (
...
/* ------------------------------------------------------------------ */
/* Event Row — extracted for cleaner key handling with fragments       */
/* ------------------------------------------------------------------ */
